...
extern Model* modelCascade;
extern Model* modelDiscourse;
extern Model* modelPareto;
extern Model* modelDilemma;
extern Model* modelDiffusion;
extern Model* modelNetwork;

// ---------------------------------------------------------------------------
// Adjacency-on-expander protocol.
//
// A Network module placed immediately to the LEFT of a compatible consumer
// (currently Diffusion) publishes its current adjacency matrix and per-node
// state through VCV Rack's expander mechanism. The consumer reads it from
// `leftExpander.module->rightExpander.consumerMessage` after verifying the
// magic header.
//
// The struct layout must be IDENTICAL in every plugin that participates
...
// Renders the standard 20HP panel labels via NanoVG. See Methods/plugin.hpp
// for the full coordinate spec; the protocol is identical across plugins.
